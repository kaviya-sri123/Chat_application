...
def index(request):
	if request.user.is_authenticated:
		try:
			...
			active_user_obj = ActiveUser.objects.all().select_related('room').get(room__id=r.id)
			...
			for user in all_active_user_in_room:
				...
				try:
					t= TextMessage.objects.filter(sender_id=user['id'],receiver=request.user).select_related('room').filter(room__id=r.id).order_by('-datetime').first()
...
def get_messages(request,sender_id):
	...
	r = Room.objects.filter(id=1).first()
	all_texts = (TextMessage.objects.filter(sender_id__in=[sender_id,receiver.id],receiver_id__in=[receiver.id,sender_id]).select_related('room').filter(room__id=r.id).select_related('sender') \
					.order_by('datetime'))

	temp =[]
	for text in all_texts:
		temp.append({'msg':text.text,'datetime':text.datetime.strftime("%d/%m/%Y, %H:%M:%S"),'sender':text.sender.username,'receiver':text.receiver.username,'sender_id':text.sender_id})
	print(temp)
